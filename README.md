...
当前行情看板默认使用 Futu OpenAPI，需要先在本机启动 FutuOpenD，默认连接 `127.0.0.1:11111`。看板暂时锁定为 `股票.txt` 中的港股和美股，后台每 30 秒检查一次，只自动刷新处于常规交易时段的市场，并把成功返回的数据保存到数据库。前端看板只读取后端已保存报价。富途快照请求会按 provider 批量合并，低于每分钟 30 次快照请求限制。
...
- 前端路由骨架：我的账户、行情看板、投资进程、历史回测、关注资产、日志、分析、设置。
- 配置文档：数据库、日志、行情 API key、前端路由、模拟交易参数。
- 账户绩效：以北京时间每日 06:00 为结算点保存账户级日/月快照，并在账户管理页展示总体、日度和月度盈亏。
- 策略学习文档：[docs/QUANT_STRATEGY_GUIDE.md](docs/QUANT_STRATEGY_GUIDE.md) 整理了策略类型、数据源、参数、模型形态和后续路线。
- 视觉原型：[design-concepts/](design-concepts/) 提供三套独立的未登录账户页风格，暂不接入现有前端。
